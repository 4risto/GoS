...
--[[ do
	local ping = 50
	local LATENCYLOGIC = (Game.Latency() <= 250 and not (Game.Latency() < 1) and not (Game.mapID == 30))
	if LATENCYLOGIC then ping = Game.Latency() end
	_G.LATENCY = ping
end ]]
_G.LATENCY = Game.Latency()
local originalGameLatency = Game.Latency
local originalGlobalLatency = _G.LATENCY
...
ChampionInfo = {
	...
	CustomIsTargetable = function(self, enemy, ally)
		...
		if Buff:HasBuff(enemy, "gwenwuntargetabilitymanager") then
			...
			if self.GwenMistObject and GetDistance(self.GwenMistObject.pos, ally.pos) >= 425 then
				return false
			end
		end
		return true
...
Cached = {
	...
	TempCacheBuffer = { m = GameTimer(), w = GameTimer(), t = GameTimer(), p = GameTimer(), s = GameTimer(), },
	TempCacheTimeout = 3, --0
	...
	WndMsg = function(self, msg, wParam)
		...
		for _, key in pairs(oKeys) do
			if (wParam == key) then
				self.TempCacheBuffer = { m = GameTimer(), w = GameTimer(), t = GameTimer(), p = GameTimer(), s =
				GameTimer(), }
				return
			end
		end
	...
	FetchSDK = function(self, SDK)
		if self.TempCacheBuffer.s < GameTimer() then
			self.TempCachedSDK = {}
			local count = #SDK
			if count and count > 0 and count < 1000 then
				for i = 1, count do
			...
			self.TempCacheBuffer.s = self.TempCacheBuffer.s + self.TempCacheTimeout
			return self.TempCachedSDK
		end
		return self.TempCachedSDK
	end,

	ResetSDK = function (self)
		if self.cacheSDK then
	...
	Reset = function(self)
		...
		if self.PetsSaved then
			for i = #self.Pets, 1, -1 do
				self.Pets[i] = nil
			end
			self.PetsSaved = false
		end
	...
	AddCachedMinion = function(self, unit)
		...
		for _, u in pairs(self.Pets) do
			if (u.networkID == unit.networkID) then
				--	print("alreadyinpetlist",Game.Timer())
				return false
			end
		end
	...
	GetMinions = function(self)
		if not self.MinionsSaved then
			...
			self.MinionsSaved = true
			local cachedPets = self:GetPets()
			local count = #cachedPets
			if count and count > 0 then
				for i = 1, count do
					local p = cachedPets[i]
					if p then
						table_insert(self.Minions, p)
					end
				end
			end
	...
	PetsTick = function(self)
		local hov = Game.GetUnderMouseObject()
		if (hov) then
			if (hov.valid and not hov.dead) then
				if (self.pet.names[hov.charName] == 1 and hov.isEnemy) then
					-- print("hov.charName: ".. hov.charName)
					self:AddCachedMinion(hov)
				end
			end
		end
		if Orbwalker.Modes[ORBWALKER_MODE_LANECLEAR] or Orbwalker.Modes[ORBWALKER_MODE_LASTHIT] or Orbwalker.Modes[ORBWALKER_MODE_HARASS] then
			if self.pet.Tick <= GameTimer() then
				local cachedPets = self:GetPets()
				local count = #cachedPets
				if count and count > 0 then
					for i = 1, count do
						local p = cachedPets[i]
						if p then
							self:AddCachedMinion(p)
						end
					end
				end
			end
			self.pet.Tick = Game.Timer() + self.pet.Delay
		end
	end,

	GetPets = function(self)
		if not self.PetsSaved then
			local cachedPets = self:FetchCachedPets()
			if cachedPets then
				local count = #cachedPets
				if count and count > 0 then --and count < 1000
					--print("Get count: ".. count)
					for i = 1, count do
						local o = cachedPets[i]
						if o and o.valid and o.visible and o.isEnemy and not o.dead and not o.isImmortal then
							table_insert(self.Pets, o)
							self.PetsSaved = true
						end
					end
				end
		...
		if self.TempCacheBuffer.p < GameTimer() then
			...
			if count and count > 0 then
				for i = 1, count do
					...
					if o and o.valid and o.isEnemy and not o.dead then
						table_insert(self.TempCachedPets, o)
					end
				end
			end
			self.TempCacheBuffer.p = self.TempCacheBuffer.p + self.TempCacheTimeout
			return self.TempCachedPets
		end
...
WriteToFile('MenuElement/Gamsteron_Spell_SummonerBoost.png',
...
WriteToFile('MenuElement/Gamsteron_Spell_SummonerDot.png',
...
WriteToFile('MenuElement/Gamsteron_Spell_SummonerExhaust.png',
...
WriteToFile('MenuElement/Gamsteron_Spell_SummonerFlash.png',
...
WriteToFile('MenuElement/Gamsteron_Spell_SummonerHaste.png',
...
WriteToFile('MenuElement/Gamsteron_Spell_SummonerHeal.png',
...
WriteToFile('MenuElement/Gamsteron_Spell_SummonerSmite.png',
...
WriteToFile('MenuElement/Gamsteron_Spell_SummonerTeleport.png',
...
WriteToFile('MenuElement/Gamsteron_TargetSelector.png',
...
Menu = {
	...
	CreateTarget = function(self)
		...
		self.Target:MenuElement({ id = 'mindistance', name = 'mindistance', value = 400, min = 100, max = 600, step = 5 })
		self.Target:MenuElement({ id = 'maxdistance', name = 'maxdistance', value = 800, min = 100, max = 1500, step = 5 })
	...
	CreateSummonerSpells = function(self)
		if self.Main.Loader.SummonerSpells:Value() then
			...
			self.SummonerSpells.Cleanse:MenuElement({ id = 'Distance', name = 'Enemies Distance < X', value = 1200, min = 0, max = 1500, step = 5 })
			self.SummonerSpells.Cleanse:MenuElement({ id = 'Duration', name = 'Buff Duration > X', value = 500, min = 0, max = 1000, step = 5 })
	...
	CreateDrawings = function(self)
		...
		self.Main.Drawings:MenuElement({
			id = 'RangeAlpha',
			name = 'AutoAttack Range Alpha',
			value = 58,
			min = 0,
			max = 100,
			identifier = "%",
	...
	CreateGeneral = function(self)
		...
		self.Main:MenuElement({ id = "callbackWHERE", name = "[Load] Callback's in", value = 2, drop = { "TICK", "DRAW", },
		callback = function(value)
			Menu.Main.SelectedCallbackOption = value
	   end
   });
		...
		self.Main:MenuElement({
			...
			callback = function(value)
				if value ~= false then
					self.UseManualLatency = value
					LATENCY = self.ManualPing
				else
...
Damage = {
	...
	ItemStaticDamage = {
		...
		[3078] = function(args)
			if Buff:HasBuff(args.From, "3078trinityforce") then --Buff:HasBuff(args.From, "sheen")
		...
		if respectPassives and from.type == Obj_AI_Hero then
			if from.charName == "Graves" then
				if target.distance < target.boundingRadius / 0.212 then
					return self:GetHeroAutoAttackDamage(from, target,
...
Data = {
	...
	--14.9 + DamageLib --DamageLib.Heros or
	HEROES = {
		Aatrox = { 3, true, 0.651 },
		Ahri = { 4, false, 0.668 },
		Akali = { 4, true, 0.625 },
...
Item = {
	...
	OnTick = function(self)
		...
		self.CachedItems = {}
		if not self.OnPreAttackLoad then
			Orbwalker:OnPreAttack(function(...) self:OnPreAttack(...) end)
			self.OnPreAttackLoad = true
		end
		if self:UseQss() then
			return
		end
	...
	OnPreAttack = function(self, args)
		if Orbwalker.Modes[ORBWALKER_MODE_COMBO] and (self:HasItem(myHero, 3084) or self:HasItem(myHero, 7025)) then
			local hsbuff = Buff:GetBuffDuration(args.Target, "3084proctracker")
			if hsbuff > 0 and hsbuff < 24997 + myHero.attackData.windUpTime + Data:GetLatency() then
				args.Process = true
				return
			end
			local enemies = Object:GetEnemyHeroes(myHero.range + myHero.boundingRadius + 50)
			if #enemies > 0 then
				for i = 1, #enemies do
					local enemy = enemies[i]
					local hsbuff = Buff:GetBuffDuration(enemy, "3084proctracker")
					if hsbuff > 0 and (enemy.valid and enemy.isTargetable and enemy.alive and enemy.visible and enemy.networkID and enemy.health > 0) and hsbuff < 24997 + myHero.attackData.windUpTime + Data:GetLatency() then
						args.Target = enemy
						return
					end
				end
...
Object = {
	...
	OnLoad = function(self)
		...
		Action:Add(function()
			local success = false
			local allysuccess = 0
			local enemysuccess = 0
			--local total = GameHeroCount()
			for i = 1, GameHeroCount() do --total
				...
				if args.valid and args.isEnemy and self.EnemyHeroesInGame[args.networkID] == nil then
					...
					enemysuccess = enemysuccess + 1
				end
				local unitsloaded = enemysuccess + allysuccess
				success = unitsloaded >= 1
			end
			-- print("success: "..success)
			-- print("total: "..total)
			return success --== total --success == total --success >= 5
		end, 1, 100)
	end,
	...
	IsHeroImmortal = function(self, unit, kill, ss, aa, sc)
		local args = {
			unit = unit,
			isAttack = kill ~= false and aa,
			ignoreUnkillable = kill or aa ~= false,
			ignoreSS = ss ~= false,
			ignoreAA = aa or kill ~= false,
			sionCheck = sc ~= false,
		}
		local hp
		hp = (100 * (args.unit.health / args.unit.maxHealth)) or 100
		self.UndyingBuffs["JaxE"] = args.isAttack
		self.UndyingBuffs["ShenWBuff"] = args.isAttack
		self.UndyingBuffs["NilahW"] = args.isAttack
		self.UndyingBuffs["willrevive"] = hp >= 50 and args.ignoreAA
		self.UndyingBuffs["kayler"] = function() return true end
		self.UndyingBuffs["undyingrage"] = function() return hp < 15 or args.IgnoreUnkillable end --args.unit.health < 100
		self.UndyingBuffs["kindredrnodeathbuff"] = function() return hp <= 11 or args.IgnoreUnkillable end
		self.UndyingBuffs["chronoshift"] = function() return hp < 15 or args.IgnoreUnkillable end
		self.UndyingBuffs["chronorevive"] = function() return hp < 15 or args.IgnoreUnkillable end
		self.UndyingBuffs["willrevive"] = function() return hp >= 50 and args.IgnoreUnkillable end
		self.UndyingBuffs["morganae"] = function() return args.ignoreSS end
		self.UndyingBuffs["fioraw"] = true
		self.UndyingBuffs["pantheone"] = true
		-- self.UndyingBuffs["jaxe"] = function() return args.ignoreAA end
		-- self.UndyingBuffs["nilahw"] = function() return args.ignoreAA end
		-- self.UndyingBuffs["shenwbuff"] = function() return args.ignoreAA end

		if args.sionCheck and args.unit.charName == "Sion" and args.unit:GetSpellData(_Q).name == "SionPassiveSpeed" then
			return true
		end
		local buffs = Buff:GetBuffs(args.unit)
		-- Iterate through buffs
		for _, buff in ipairs(buffs) do
			if buff.count > 0 then
				local buffName = buff.name:lower()

				if (args.sionCheck) then
				...
				for buffKey, buffCondition in pairs(self.UndyingBuffs) do
					if buffCondition and Buff:HasBuff(args.unit, buffName) then
						local buffData = Buff:GetBuff(args.unit, buffName)
						if args.isAttack then
							if myHero.charName == "Azir" then
								return false
							end
							if buffData then
								local attackCalc = myHero.attackData.windUpTime +
								(args.unit.distance / Attack:GetProjectileSpeed()) + Data:GetLatency() / 2
								if buffData.duration >= attackCalc then -- >=
									return true
								else
								end
							end
		...
		if args.ignoreSS and Buff:HasBuffTypes(args.unit, { [4] = true }) then
			return true
		end
		return false
	end,
	...
	GetOtherMinions = function(self, range, bbox, immortal) --Wards
		local result = {}
		local a = self:GetOtherAllyMinions(range, bbox, immortal) --GetAllyWards
		local b = self:GetOtherEnemyMinions(range, bbox, immortal) --GetEnemyWards
		for i = 1, #a do
			table_insert(result, a[i])
		end
		for i = 1, #b do
	...
	GetOtherAllyMinions = function(self, range) --GetAllyWards
		local result = {}
	...
	GetOtherEnemyMinions = function(self, range) --GetEnemyWards
		local result = {}
		local cachedwards = Cached:GetWards()
		for i = 1, #cachedwards do
			local obj = cachedwards[i]
...
Target = {
	...
	GetTargets = function(self, a, dmgType, isAttack)
		...
		-- return (#a == 0 and nil or (a or a[1])) --??
		-- return (#a == 0 and nil or (a and a[1])) --?
		-- return (#a == 0 and nil or a) --Targets?
		-- return (#a == 0 and nil or a[1]) --Target?
...
Target.SortModes = {
	...
	[SORT_CLOSEST] = function(a, b) --2
		return a.distance < b.distance
	end,

	[SORT_NEAR_MOUSE] = function(a, b) --3
		return GetDistance(a.pos, Vector(mousePos)) < GetDistance(b.pos, Vector(mousePos))
	end,

	[SORT_LOWEST_HEALTH] = function(a, b) --4
		return a.health < b.health
	end,

	[SORT_LOWEST_MAX_HEALTH] = function(a, b) --5
		return a.maxHealth < b.maxHealth
	end,

	[SORT_HIGHEST_PRIORITY] = function(a, b) --6
		return Target:GetPriority(a) > Target:GetPriority(b)
	end,

	[SORT_MOST_STACK] = function(a, b) --7
		local aMax = 0
	...
	[SORT_MOST_AD] = function(a, b) --8
		return a.totalDamage > b.totalDamage
	end,

	[SORT_MOST_AP] = function(a, b) --9
		return a.ap > b.ap
	end,

	[SORT_LESS_CAST] = function(a, b) --10
		local aMultiplier = 1.75 - Target:GetPriority(a) * 0.15
		local bMultiplier = 1.75 - Target:GetPriority(b) * 0.15
		local aDef, bDef = 0, 0
		local magicPen, magicPenPercent = myHero.magicPen, myHero.magicPenPercent
	...
	[SORT_LESS_ATTACK] = function(a, b) --11
		local aMultiplier = 1.75 - Target:GetPriority(a) * 0.15
		local bMultiplier = 1.75 - Target:GetPriority(b) * 0.15
		local aDef, bDef = 0, 0
		local armorPen, bonusArmorPenPercent = myHero.armorPen, myHero.bonusArmorPenPercent
...
Health = {
	...
	OnTick = function(self)
		...
		for i = 1, #self.CachedMinions do
			...
			elseif team == Data.EnemyTeam then
				...
				then
					...
					if (Cached.pet.names[obj.charName] == 1 and obj.isEnemy) then
						local value = {
							LastHitable = true,
							Unkillable = false,
		...
		--time = windup - self.ExtraFarmDelay:Value() * 0.001 - Data:GetLatency() --why is this -getlatency here? i isbjorn might try removing it -- - Data:GetLatency()
		time = windup - self.ExtraFarmDelay:Value() * 0.001 + Data:GetLatency()
	...
	GetLaneClearTarget = function(self)
		...
		local structure = #self.EnemyStructuresInAttackRange > 0 and self.EnemyStructuresInAttackRange[1] or nil
		local ward = #self.EnemyWardsInAttackRange > 0 and self.EnemyWardsInAttackRange[1] or nil
		if structure ~= nil then
			if not LastHitPriority then
				return structure
			end
...
do
	...
	_G.Control.Attack = function(target)
		if target then
			Cursor:Add(MOUSEEVENTF_RIGHTDOWN, target)
	...
	and not (ping < 1)
	and not (mapID == 30)
	and not (Menu.Main.UseManualLatency:Value()) then
		ping = Game.Latency()
		return ping
	end
	_G.LATENCY = ping ]]
...
Cursor = {
	...
	StepPressKey = function(self)
		if self.CastPos.type then
			...
			elseif self.CastPos.type == Obj_AI_Hero then
				if not Control.IsKeyDown(HK_TCO) then
					Control.IsKeyDown(HK_TCO)
				end
			end
		end
...
Attack = {
	...
	GetWindup = function(self)
		...
		if self.IsGraves then
			return myHero.attackData.windUpTime * 0.005 -- 0.20
		end
	...
	GetAnimation = function(self)
		...
		if self.IsAphelios and Buff:HasBuff(myHero, "ApheliosCrescendumManager") then
			return 1.1067 / (myHero.attackSpeed * self.BaseAttackSpeed) -- 6.67% if crit
		end
	...
	IsReady = function(self)
		...
		if myHero.charName == "Rengar" and Game.CanUseSpell(_Q) ~= 8 and myHero:GetSpellData(63).castTime + myHero.attackData.windDownTime > Game.Timer() + 10 + Data:GetLatency() then
			return false
		end
...
local setCallback = function(selected, ticks, draws, wndmsgs)
	local option = selected or Menu.Main.SelectedCallbackOption
	if option == 3 then
...
Callback.Add("Load", function()
	ChampionInfo:OnLoad()
	Object:OnLoad()
	-- Cached:GetSDK() --TODO: Save base SDK, temp load/combine all SDK callbacks
	...
	--[[ local ping = Menu.Latency
	local LATENCYLOGIC = (Game.Latency() <= 250 and not (Game.Latency() < 1) and not (Game.mapID == 30))
	if LATENCYLOGIC and not Menu.UseManualLatency then ping = Game.Latency() end
	LATENCY = ping ]]
	local option = Menu.Main.callbackWHERE:Value()
	setCallback(option, ticks, draws, wndmsgs)
