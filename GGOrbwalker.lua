	...
	HeroStaticDamage = {
		...
		["Caitlyn"] = function(args)
			if Buff:HasBuff(args.From, "caitlynpassivedriver") then
				local modCrit = 1.09375 + (Item:HasItem(args.From, 3031) and 0.21875 or 0)
				if args.TargetIsMinion then
					args.RawPhysical = args.RawPhysical + (1 + (modCrit * args.From.critChance)) * args.From.totalDamage
				else
					local unitLevel = args.From.levelData.lvl
					local modLevel = unitLevel < 7 and 0.5 or (unitLevel < 13 and 0.75 or 1)
					args.RawPhysical = args.RawPhysical
						+ (1 + (modCrit * args.From.critChance)) * args.From.totalDamage * modLevel
				end
			end
		end,
		["Corki"] = function(args)
		...
		["Nasus"] = function(args)
			if Buff:HasBuff(args.From, "NasusQ") then
				args.RawPhysical = args.RawPhysical
					+ math_max(Buff:GetBuffCount(args.From, "NasusQStacks"), 0)
					+ 10
		...
		["Vayne"] = function(args)
			if Buff:HasBuff(args.From, "vaynetumblebonus") then
				args.RawPhysical = args.RawPhysical
					+ (0.25 + 0.05 * args.From:GetSpellData(_Q).level) * args.From.totalDamage
			end
		end,
	},
		...
		[3087] = function(args)
			if Buff:GetBuffCount(args.From, "itemstatikshankcharge") == 100 then
				...
				args.RawMagical = args.RawMagical
					...
					* t[math_max(math_min(args.From.levelData.lvl, 18), 1)]
			end
		end,
		[3091] = function(args)
			args.RawMagical = args.RawMagical + 40
...
Target = {
	...
	GetTargets = function(self, a, dmgType, isAttack)
		...
		if self.CurrentSortMode == nil then
			return nil
		end
		return (#a == 0 and nil or (a or a[1])) --??
		-- return (#a == 0 and nil or (a and a[1])) --?
		-- return (#a == 0 and nil or a) --now?
		-- return (#a == 0 and nil or a[1]) --old?
	end,
	...
	_G.Control.CastSpell = function(key, a, b, c)
		...
		if a and (a.x or a[1]) then
			if GetDistance(Game.mousePos(), pos) < 2 then
				--What's this for? -Imp
				return true --was commented?
			end
		end
		if not b then
			if not (Vector(pos):To2D().onScreen) then
				return false
			end
		end
		return false
...
Cursor = {
	...
	StepPressKey = function(self)
		if self.CastPos.type then
			if self.CastPos.type ~= "AIHeroClient" then
				Control.KeyUp(HK_TCO)
			elseif self.CastPos.type == "AIHeroClient" then
				Control.KeyDown(HK_TCO)
			end
		end
